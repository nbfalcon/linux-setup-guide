...
* Install basic apps
#+begin_src shell
sudo dnf install keepassxc emacs neovim
#+end_src
* Install useful apps
#+begin_src shell
sudo dnf install ksysguard kgraphviewer cantor maxima maxima-runtime-sbcl thunderbird okular filelight
#+end_src
* Java Development
Since Linux is most useful for software devs, and Java is very common on Linux,
add that as well.
#+begin_src shell
sudo dnf install java-17-openjdk-devel
#+end_src
* Install useful apps and dependencies (CLI)
#+begin_src shell
sudo dnf install htop fd-find ripgrep ripgrep graphviz
#+end_src
Note that DOOM Emacs depends on fd-find and ripgrep.
...
#+end_src
* Cannot open UART/USB ttys (permission denied) :fixErrors:
Your user needes to be added to the ~dialout~ group.
#+begin_src shell
